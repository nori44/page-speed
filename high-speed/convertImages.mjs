...
program
  ...
  .option("-w, --avif", "avif化を行う", false)
  .option("-a, --avif-suffix-add", "avif化の際、拡張子を書き換え（false）するか追加（true）するか", false)
...
const ENCODE_AVIF = Options.avif;
const AVIF_SUFFIX_ADD = Options.avifSuffixAdd;
...
await Promise.all(
  imageFileList.map(async imagePath => {
    ...
    if (encoder !== "") {
      ...
      if (ENCODE_AVIF) {
        encodeOptions["avif"] = ENCODER_OPTIONS["avif"];
      }
    ...
    if (isCopy) {
      ...
      if (ENCODE_AVIF) {
        // avif と encodeOptions を指定して最適化
        const destinationPathAvif = AVIF_SUFFIX_ADD
          ? destinationPath + ".avif"
          : destinationPath.slice(0, fileExtension.length * -1) + "avif";

        await sharp(sourcePath)
          .avif(ENCODER_OPTIONS["avif"][encoder])
          .toFile(destinationPathAvif);
        if (action !== "") {
          action += " and ";
        }
        action += "encoded to avif";
      }
    }
